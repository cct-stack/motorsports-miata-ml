...
class LapSimulator:
    ...
    def __init__(self, vehicle: VehicleModel, track: TrackModel,
                 v_cap: float = 100.0, grip_scale=None):
        self.vehicle = vehicle
        self.track = track
        self.v_cap = v_cap  # upper bound on straight-line speed search [m/s]
        # Optional per-point lateral-grip multiplier (length = #track points).
        # 1.0 everywhere = unchanged QSS. The transient layer
        # (transient_lap.py) supplies values < 1 where damper-driven load
        # variation costs grip; None means a flat array of ones.
        n = len(self.track.curvature)
        if grip_scale is None:
            self.grip_scale = np.ones(n)
        else:
            self.grip_scale = np.asarray(grip_scale, dtype=float)
            if self.grip_scale.shape != (n,):
                raise ValueError("grip_scale must match the number of track "
                                 f"points ({n}), got {self.grip_scale.shape}")
    ...
    def _corner_speed_limit(self) -> np.ndarray:
        ...
        sections (kappa ~ 0) the limit is the search cap. The per-point
        ``grip_scale`` multiplies the available lateral grip."""
        kappa = np.maximum(np.abs(self.track.curvature), 1e-9)
        ay = self.vehicle.max_lat_accel(0.0) * self.grip_scale
        v = np.minimum(np.sqrt(ay / kappa), self.v_cap)
        for _ in range(8):
            ay = self.vehicle.max_lat_accel(v) * self.grip_scale
            v_new = np.minimum(np.sqrt(ay / kappa), self.v_cap)
    ...
    def solve(self) -> Tuple[float, np.ndarray]:
        ...
        for _ in range(passes):
            for i in range(n):
                ...
                ay_max = self.vehicle.max_lat_accel(vp) * self.grip_scale[j]
        ...
        for _ in range(passes):
            for i in range(n - 1, -1, -1):
                ...
                ay_max = self.vehicle.max_lat_accel(vn) * self.grip_scale[j]
